...
function viewComments(serviceOrderId) {
  ...
  loadServiceComments(serviceOrderId);
}

async function loadServiceComments(serviceOrderId) {
  if (clientAuth.getUser() && clientAuth.getUser().demo) {
    alert('No comments in demo account.');
    return;
  }
  try {
    const response = await api.getServiceComments(serviceOrderId);
    if (response.comments && response.comments.length > 0) {
      const lines = response.comments.map(c => {
        const email = escapeHtml(c.user_email || 'User');
        const comment = escapeHtml((c.comment || '').substring(0, 80));
        const truncated = (c.comment || '').length > 80 ? '...' : '';
        return `${email}: ${comment}${truncated}`;
      }).join('\n');
      alert('Comments:\n\n' + lines);
    } else {
      alert('No comments for this service yet.');
    }
  } catch (error) {
